...
export function AlphabetEditor({ dfa, onUpdate }) {
    const [isAdding, setIsAdding] = useState(false);
    const [newSymbol, setNewSymbol] = useState('');

    const handleAddSymbol = () => {
        if (newSymbol.trim() && newSymbol.trim().length === 1) {
            if (dfa.alphabet.includes(newSymbol.trim())) {
                alert(`Symbol "${newSymbol}" already exists in alphabet`);
                return;
            }
            dfa.setAlphabet([...dfa.alphabet, newSymbol.trim()]);
            setNewSymbol('');
            setIsAdding(false);
            onUpdate();
        } else {
    ...
    const handleRemoveSymbol = (symbol) => {
        if (window.confirm(`Remove symbol "${symbol}" from alphabet?`)) {
            dfa.setAlphabet(dfa.alphabet.filter(s => s !== symbol));
            onUpdate();
        }
    };
